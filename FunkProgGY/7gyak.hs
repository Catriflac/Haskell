...
-- UNIVERZÁLIS REKURZÍV FX KEZELŐ
mymap :: (a -> b) -> [a] -> [b]
mymap f [] = []
mymap f (x:xs) = f x : mymap f xs

toString2 :: [Int] -> String
toString2 xs = mymap chr xs
-- toString2 = mymap chr <- ugyanúgy jó 


double2 xs = mymap (\x -> if even x then 2*x else x) xs -- \ a lambda rekurzióhoz 
...
-- mappal es filterrel?
doubleEvens' :: [Int] -> [Int]
doubleEvens' xs = map (\x -> 2 * x) (filter even xs)
